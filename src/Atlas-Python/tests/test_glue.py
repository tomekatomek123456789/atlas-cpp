    ...
    for id in point_ids:
        ...
        if obj_id==obj.id:
            ...
            if vector==vector2:
                obj_ok = 1
                if debug_flag:
                    print("this ok: %s: %s with %s glue_id from %s" % (obj_id, vector, id, point_ids),end='')
            else:
                if debug_flag:
                    print("not this one: %s: %s!=%s with %s glue_id from %s" % (obj_id, vector, vector2, id, point_ids),end='')
        else:
            if vector==vector2:
                result.append(obj_id)
                if debug_flag:
                    print("other ok: %s <-> %s: %s with %s glue_id from %s" % (obj.id, obj_id, vector, id, point_ids),end='')
            else:
                if debug_flag:
                    print("other not ok: %s <-> %s: %s!=%s with %s glue_id from %s" % (obj.id, obj_id, vector, vector2, id, point_ids),end='')
                all_ok = 0
...
def process_glue(msg, obj, point_id, other_id, id_dict, to_process, check_id=1):
    if id_dict.has_key(other_id):
        ...
        if check_id:
            ...
            if debug_flag:
                print("%s: %s <-%s-> %s" % (msg, obj.id, point_id, obj2.id),end='')
        else:
            if debug_flag:
                print("%s: %s:%s <-> %s:%s" % (msg, obj.id, point_id, obj2.id, point_id2),end='')

        ...
        if c.is_name:
            ...
            if c.name!=None:
                if not isinstance(obj, Object):
                    raise_(BinaryException, "attribute outside mapping (%s:%s)!" % \
                          (c.name, c.value))
                setattr(obj, c.name, c.value)
            else:
                if type(obj)!=ListType:
                    raise_(BinaryException, "object not inside list (%s)!" % c.value)
                obj.append(c.value)
